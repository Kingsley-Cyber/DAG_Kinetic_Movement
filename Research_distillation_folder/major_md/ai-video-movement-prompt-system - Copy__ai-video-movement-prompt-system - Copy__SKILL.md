...
## Combat / fight choreography (the mathematical metrics layer)

CPCS extends from talking-head UGC into **fight and action sequences** by adding a mathematical
metrics layer on top of the same FACS + Laban + body-movement foundation. Fight choreography
requires quantitative precision that UGC doesn't — biomechanical kinematics, frame-level timing
ratios, spatial geometry, power escalation curves, and constraints with numerical tolerances.

**Full reference:** `references/combat_choreography.md`

Combat uses a **two-document architecture**:
- **Authoring layer** (YAML): `assets/combat_control_package.template.yaml` — creative direction
  (beats, FACS, Laban categorical, body mechanics prose, compiled prompt)
- **Kinematic data layer** (JSON): `assets/combat_kinematics.template.json` — motion solver output
  (joint xyz tracks, typed contact events, Laban continuous floats, camera spatial path, verification
  metrics, provenance links)

The authoring doc says WHAT happens; the kinematic doc says WHERE and WHEN in coordinate space.
Provenance links connect them. The compiled prompt is generated from both.

**Worked examples:**
- `lab/variants/naruto_sasuke_rooftop_clash.yaml` (authoring layer — 10s shonen fight)
- `assets/combat_kinematics.fast_exchange.example.json` (kinematic layer — 8s kung fu exchange)

### What the combat template adds over the UGC template:

1. **Kinematics per beat** — closing speed (m/s), strike velocity, screen velocity (% of screen/s),
   gap distances, torso rotation degrees, recoil distance, flip arc height, projectile velocity.
2. **Frame timing per strike** — anticipation : contact : follow-through : hold frame ratios for
   every strike, validated against the beat's frame budget (`frames = (end_s - start_s) × fps`).
3. **Spatial geometry** — arena dimensions in meters, engagement range, elevation, max aerial height,
   environmental obstacles.
4. **Tempo / BPM** — fight rhythm per beat with an escalation curve (not flat — tension and release).
5. **Power curve** — normalized 0–1 force scaling across beats; must generally escalate (dips
   allowed for tension contrast).
6. **Character profiles** — height, reach, weight, fighting archetype (brawler/interceptor/etc.),
   stance, dominant hand. These drive the kinematics.
7. **FACS at combat intensity** — AU combos shift up to C–E range (vs. B–C for UGC), expression
   arcs mirror the power curve, contrasting registers between fighters.
8. **Laban strike mapping** — each strike type maps to a named Effort (Thrust for straights, Slash
   for hooks, Flick for evasion, Press for power-ups). The climax is the bound→free transition.
9. **Camera with quantified parameters** — focal length mm, angle degrees, tracking speed match,
   shake amplitude/frequency/decay, whip-pan speed.
10. **Mathematical constraints with tolerances** — frame budget validation, contact accuracy (<0.05m),
    identity preservation (≥0.95), power escalation monotonicity, held impact minimums, facial
    expression max-hold (1.0s), audio sync (±2 frames).

### Workflow for fight sequences:

1. Define the **arc** — label each beat (approach → exchange → evasion → charge → commit → collision).
2. Set the **tempo curve** — BPM per beat, with dips for contrast.
3. Set the **power curve** — normalized 0–1 per beat, escalating.
4. Build **character profiles** with body math and fighting archetypes.
5. Define **spatial geometry** — arena, distances, obstacles.
6. Fill each beat: kinematics → frame timing → FACS → Laban → body mechanics → camera → audio.
7. Validate **constraints** — frame budget sums, contact accuracy, escalation monotonicity.
8. **Compile to prose** with the key numbers baked in (degrees, meters, frame counts, speeds).
