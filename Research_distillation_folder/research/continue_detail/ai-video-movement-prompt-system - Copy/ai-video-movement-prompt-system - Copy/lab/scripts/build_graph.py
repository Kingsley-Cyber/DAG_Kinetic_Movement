...
def build(root: Path) -> dict:
    ...
    # concept cards
    for line in (lab / "concepts.jsonl").read_text(encoding="utf-8").splitlines():
        if not line.strip():
            continue
        c = json.loads(line)
    ...
    # blocks
    blocks = yaml.safe_load((lab / "blocks.yaml").read_text(encoding="utf-8")) or {}
    ...
    reg = yaml.safe_load((lab / "registry.yaml").read_text(encoding="utf-8")) or {}
    ...
    # runs
    for row in csv.DictReader((lab / "runs" / "results.csv").open(encoding="utf-8", newline="")):
