...
def main() -> None:
    ...
    current = gpath.read_text(encoding="utf-8") if gpath.exists() else ""
    if current != rebuilt:
        if fix:
            gpath.write_text(rebuilt, encoding="utf-8")
    ...
    cards_text = (lab / "concepts.jsonl").read_text(encoding="utf-8")
    index_text = (lab / "CONCEPT_INDEX.md").read_text(encoding="utf-8")
    for pkg in packages:
    ...
    agents = (lab / "AGENTS.md").read_text(encoding="utf-8")
