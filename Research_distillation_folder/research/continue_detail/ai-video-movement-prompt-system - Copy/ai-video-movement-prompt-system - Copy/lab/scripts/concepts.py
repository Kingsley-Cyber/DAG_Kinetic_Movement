...
def load_cards(root: Path) -> list[dict]:
    cards = []
    for n, line in enumerate((root / "lab" / "concepts.jsonl").read_text(encoding="utf-8").splitlines(), 1):
        if not line.strip():
            continue
        try:
...
def cmd_validate(_args, cards) -> None:
    ...
    try:
        ...
        import csv
        known |= {r["run_id"] for r in csv.DictReader((find_root() / "lab/runs/results.csv").open(encoding="utf-8", newline=""))}
    except ImportError:
